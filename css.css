...
class EmotionObject{
    ...
    getOnomatopoeiaWords(){
        ...
        for(let i=0; i < this.onomatopoeia.length; i++){
            let ono = this.onomatopoeia[i]
            res.push(new Word(ono, dictionary[ono], pictureDictionary[ono]))
        }
        return res
    }
    ...
    getHtmlContainerString(){
        let httmlString =  `<div id = ${this.emotion}
        style = "background: ${this.color}">
            <div class="container py-3">
                <div class = "p-3 text-white">
                    <h2  id="emotion">${this.emotion}</h2>
                    <p>${this.description}</p>
                </div>
            <div class="d-flex justify-content-between flex-wrap">`
        
        for(let i = 0; i < this.getOnomatopoeiaWords().length;i++){
            let obj = this.getOnomatopoeiaWords()[i]

            httmlString +=`
                            <div class ="col-md-5 col-12 bg-white px-0 my-2 d-flex">
            <div class="col-8">
                <h4 class="pt-3">${obj.word}</h4>
                <p class = "pt-2">${obj.defintion}</p>
                </div>
            <div class = "d-flex col-4 justify-content-center align-items-center px-0">
                <img class="col-12 p-1 imgFit" src = ${obj.pictureUrl}>
            </div>
        </div>`
        }
            httmlString += `</div>
...
class helperFunc{
    ...
    static generateCategoryElements(emotionObjectlist){
        ...
        container.classList.add("d-flex", "justify-content-center", "container", "flex-wrap")

        for (let i=0; i<emotionObjectlist.length;i++){
            let obj = emotionObjectlist[i]

            let content = document.createElement("div")
            content.classList.add("text-center", "expandlink", "m-4", "p-4", "col-12", "col-md-6", "col-lg-3")
            content.style.backgroundColor =  `${obj.color}`

            content.innerHTML =`
                    <a href="#${obj.emotion}"></a>
                    <h3 class="text-white">${obj.emotion}</h3>
                    <h1>"🥳"</h1>
                    <p class="text-white">${obj.description}</p>
...
helperFunc.generateCategoryElements(emotions);
helperFunc.generateOnomatopoeiaElements(emotions);